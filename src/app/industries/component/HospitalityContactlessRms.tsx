  ...
  return (
    <section className="font-sora">
      ...
      <div className="bg-[#2776ea] py-24 text-white relative overflow-hidden">
        <div className="container mx-auto px-6 max-w-7xl relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black mb-4">{rms.title}</h2>
            <p className="text-white/80 font-bold text-xl">{rms.pricing.title}</p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 mb-16">
            ...
            <div className="bg-white/10 border border-white/20 p-8 rounded-[2.5rem] backdrop-blur-md">
              ...
              <p className="text-blue-50 mb-6">{rms.pricing.forecasting.description}</p>
              
              <div className="mb-6">
                <h4 className="font-bold text-white mb-3 text-sm uppercase">{rms.pricing.forecasting.inputs.title}</h4>
                <ul className="grid sm:grid-cols-2 gap-2">
                  {rms.pricing.forecasting.inputs.items.map((item, i) => (
                    <li key={i} className="text-xs text-blue-100 border-l border-white/30 pl-2">{item}</li>
                  ))}
                </ul>
              </div>

              <div className="bg-white text-[#2776ea] p-4 rounded-xl font-bold text-center shadow-lg">
            ...
            <div className="bg-white/10 border border-white/20 p-8 rounded-[2.5rem] backdrop-blur-md">
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="font-bold text-white mb-2">{rms.pricing.recommendations.dynamic.title}</h4>
                  <ul className="space-y-1">
                    {rms.pricing.recommendations.dynamic.items.map((item, i) => (
                      <li key={i} className="text-sm text-blue-50">• {item}</li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="bg-[#76ea27] rounded-[2rem] p-8 text-slate-900 shadow-xl">
              ...
              <ul className="space-y-2">
                {rms.platforms.map((plat, i) => (
                  <li key={i} className="text-sm font-bold border-b border-slate-900/10 last:border-0 pb-2 last:pb-0">
                    {plat}
                  </li>
                ))}
              </ul>
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-[2rem] p-8 hover:shadow-xl transition-all">
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="font-bold text-slate-700 mb-2 border-b border-slate-200 pb-1">{restaurant.pos.orderMgmt.title}</h4>
                  <ul className="space-y-1">
                    {restaurant.pos.orderMgmt.items.map((item, i) => <li key={i} className="text-sm text-slate-600">• {item}</li>)}
                  </ul>
                </div>
                <div>
                  <h4 className="font-bold text-slate-700 mb-2 border-b border-slate-200 pb-1">{restaurant.pos.payment.title}</h4>
                  <ul className="space-y-1">
                    {restaurant.pos.payment.items.map((item, i) => <li key={i} className="text-sm text-slate-600">• {item}</li>)}
                  </ul>
                </div>
              </div>
            </div>
      ...
      <div className="bg-slate-50 py-24">
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6">{travel.title}</h2>
