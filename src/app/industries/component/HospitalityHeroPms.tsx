    ...
    <section className="font-sora bg-white overflow-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div className="relative hidden lg:flex justify-end items-center">
              ...
              <div className="relative z-10 w-full max-w-[340px] bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_20px_50px_rgb(0,0,0,0.06)] overflow-hidden">
                ...
                <div className="bg-[#2776ea] p-6 text-white relative overflow-hidden">
                   ...
                   <div className="flex justify-between items-start relative z-10">
                      <div>
                         <p className="text-caption font-bold text-blue-100 uppercase tracking-wide mb-1">Welcome Back</p>
                ...
                <div className="p-6">
                   <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col items-center gap-2 hover:bg-white hover:shadow-lg transition-all cursor-pointer">
                         ...
                         <span className="text-caption font-bold text-slate-600">Check-In</span>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col items-center gap-2 hover:bg-white hover:shadow-lg transition-all cursor-pointer">
                         <Utensils size={20} className="text-[#2776ea]" />
                         <span className="text-caption font-bold text-slate-600">Dining</span>
                      </div>
                   </div>

                   <div className="bg-slate-900 rounded-2xl p-4 flex items-center justify-between text-white">
                      <div>
                         <p className="text-caption font-bold text-slate-400 uppercase">Room Key</p>
              ...
              <div className="absolute -bottom-6 -left-6 bg-white p-4 rounded-2xl border border-slate-100 shadow-xl z-20 flex items-center gap-4 animate-[float_4s_ease-in-out_infinite]">
                ...
                <div>
                  <span className="block text-caption font-bold text-slate-400 uppercase tracking-wider">Rating</span>
                  <span className="block text-submenu font-black text-slate-900">4.9/5.0</span>
                </div>
              </div>
            </div>
      ...
      <div className="py-24 bg-white border-t border-slate-100">
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <div className="mb-24">
            <h3 className="text-h2 font-black text-slate-900 mb-10 text-center">Critical Hospitality Challenges</h3>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {overview.challenges.map((item, index) => (
                <div key={index} className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300">
                  ...
                  <p className="text-submenu text-slate-600 leading-relaxed font-medium">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#2776ea] rounded-[3rem] p-10 md:p-16 shadow-2xl relative overflow-hidden text-white">
            <div className="relative z-10">
              <div className="text-center max-w-3xl mx-auto mb-12">
                ...
                <p className="text-body text-blue-50 leading-relaxed">{overview.expertise.description}</p>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                {overview.expertise.stats.map((stat, index) => (
                  <div key={index} className="flex items-center gap-4 p-6 rounded-3xl bg-white/10 border border-white/10 backdrop-blur-md">
                    ...
                    <span className="text-submenu font-bold text-white">{stat}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-24">
            ...
            <div>
              <div className="flex items-center gap-4 mb-10 pb-4 border-b border-slate-200">
                ...
                <h3 className="text-h2 font-black text-slate-900">{pms.frontOffice.title}</h3>
              </div>
              <div className="grid lg:grid-cols-3 gap-8">
                {pms.frontOffice.columns.map((col, idx) => (
                  <div key={idx} className="bg-white p-8 rounded-[2rem] border border-slate-200 hover:border-[#2776ea]/30 transition-colors shadow-sm">
                    <h4 className="font-bold text-h3 text-slate-900 mb-6">{col.title}</h4>
            ...
            <div className="bg-[#2776ea] rounded-[3rem] p-10 md:p-16 text-white relative overflow-hidden shadow-2xl">
              <div className="relative z-10">
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <h3 className="text-h2 font-black">{pms.housekeeping.title}</h3>
                </div>

                <div className="grid lg:grid-cols-3 gap-12">
                  <div>
                    <h4 className="font-bold text-[#76ea27] mb-6 uppercase tracking-wider text-caption">{pms.housekeeping.statusMgmt.title}</h4>
                    <ul className="space-y-3">
                      {pms.housekeeping.statusMgmt.items.map((item, i) => (
                        <li key={i} className="flex items-start gap-3 text-submenu text-blue-50 font-medium">
                  ...
                  <div>
                    <h4 className="font-bold text-white mb-6 uppercase tracking-wider text-caption">{pms.housekeeping.mobileApp.title}</h4>
                    <ul className="space-y-3">
                  ...
                  <div className="bg-white/10 p-8 rounded-3xl border border-white/20 backdrop-blur-md">
                    ...
                    <ul className="space-y-4">
                      {pms.housekeeping.efficiency.items.map((item, i) => (
                        <li key={i} className="flex items-start gap-3 text-submenu font-bold text-white">
            ...
            <div className="grid lg:grid-cols-2 gap-12">
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h3 className="text-h2 font-black text-slate-900">{pms.pos.title}</h3>
                </div>
                  ...
                  {pms.pos.sections.map((sec, idx) => (
                    <div key={idx}>
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <h4 className="font-bold text-slate-900 text-h3">{sec.title}</h4>
                      </div>
                      <ul className="space-y-2 pl-7 border-l-2 border-[#2776ea]/20">
                        {sec.items.map((item, i) => (
                          <li key={i} className="text-submenu text-slate-600 font-medium">{item}</li>
                        ))}
                      </ul>
                    </div>
                  ))}
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h3 className="text-h2 font-black text-slate-900">{pms.backOffice.title}</h3>
                </div>
                <div className="space-y-8">
                  {pms.backOffice.sections.map((sec, idx) => (
                    <div key={idx}>
                      <h4 className="font-bold text-slate-900 mb-4 text-h3">{sec.title}</h4>
            ...
            <div className="text-center">
              <span className="text-caption font-bold text-slate-500 uppercase tracking-wider block mb-4">PMS Platforms</span>
              <div className="flex flex-wrap justify-center gap-3">
                {pms.platforms.map((plat, i) => (
                  <span key={i} className="px-4 py-2 bg-white border border-slate-200 rounded-full text-submenu font-bold text-slate-600 shadow-sm hover:border-[#2776ea] transition-colors">
                    {plat}
                  </span>
                ))}
              </div>
